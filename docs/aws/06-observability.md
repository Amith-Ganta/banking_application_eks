...
exactly like `http://banking-platform.example/argocd` works today. Create one IngressRoute
...
spec:
  entryPoints: [web]
  routes:
    - match: Host(`banking-platform.example`) && PathPrefix(`/grafana`)
      kind: Rule
      priority: 100
      services:
...
spec:
  entryPoints: [web]
  routes:
    - match: Host(`banking-platform.example`) && PathPrefix(`/prometheus`)
      kind: Rule
      priority: 100
      services:
...
spec:
  entryPoints: [web]
  routes:
    - match: Host(`banking-platform.example`) && PathPrefix(`/alertmanager`)
      kind: Rule
      priority: 100
      services:
...
| Tool         | URL                                     | Login             |
|--------------|-----------------------------------------|-------------------|
| Grafana      | `http://banking-platform.example/grafana`      | `admin` / `admin` |
| Prometheus   | `http://banking-platform.example/prometheus`   | none              |
| Alertmanager | `http://banking-platform.example/alertmanager` | none              |

```bash
curl -s  http://banking-platform.example/prometheus/-/healthy       # Prometheus Server is Healthy.
curl -s  http://banking-platform.example/alertmanager/-/healthy     # OK
curl -s  http://banking-platform.example/grafana/api/health         # {"database":"ok", ...}  (JSON, not HTML)
```
...
> prometheus:
>   prometheusSpec:
>     externalUrl: http://banking-platform.example/prometheus   # http for Phase 6; Phase 7 flips to https
> alertmanager:
>   alertmanagerSpec:
>     externalUrl: http://banking-platform.example/alertmanager
> grafana:
>   grafana.ini:
>     server:
>       root_url: http://banking-platform.example/grafana
...
| Tool         | URL                                      | Login             |
|--------------|------------------------------------------|-------------------|
| Grafana      | `https://banking-platform.example/grafana`      | `admin` / `admin` |
| Prometheus   | `https://banking-platform.example/prometheus`   | none              |
| Alertmanager | `https://banking-platform.example/alertmanager` | none              |

> HTTP auto-redirects to HTTPS, so typing `http://…` still lands on the secure URL.

**Quick health check from the terminal:**
```bash
curl -sI https://banking-platform.example/grafana/login      | head -1   # HTTP/2 200
curl -s  https://banking-platform.example/prometheus/-/healthy           # Prometheus Server is Healthy.
curl -s  https://banking-platform.example/alertmanager/-/healthy         # OK
```
...
`banking-platform.example` — plus the sub-path serving config each app needs
...
Open the Prometheus UI at **`https://banking-platform.example/prometheus/`**, paste a
...
a row for testing, hit a bad endpoint: `curl -X POST https://banking-platform.example/api/v1/auth/login -d 'bogus'`.
...
Open **`https://banking-platform.example/alertmanager/`** to see firing alerts, silences,
