...
**Goal:** Serve the platform on the **bare apex `banking-platform.example`** (no
...
> After:  `http://banking-platform.example/`  and  `http://banking-platform.example/argocd/`

> Set this once and reuse it in every command below:
> ```bash
> export HOSTNAME_APP="banking-platform.example"   # 👈 your apex domain
> ```
...
- The **apex** (`banking-platform.example`) **cannot** be a `CNAME` — and **GoDaddy DNS
...
```
GoDaddy (registrar)  --delegate NS-->  Route 53 hosted zone (banking-platform.example)
...
# Non-authoritative answer: Name: banking-platform.example  Address: 52.x.x.x (and more)
```

If they disagree across regions, check https://dnschecker.org (paste
`banking-platform.example`). Then a quick HTTP probe:
...
ingress:
  ...
  className: traefik
  host: banking-platform.example       # 👈 YOUR apex domain
  tls: false                    # Phase 7 flips this to true
  tlsSecretName: banking-tls
  clusterIssuer: letsencrypt-prod
```

That one Ingress routes, on `banking-platform.example`:
- `/api` → `api-gateway`
- `/` → `frontend`
...
> `http://banking-platform.example/argocd/` will work too — the more-specific
...
git commit -m "phase 5: expose the platform on banking-platform.example"
...
Open in a browser:
- **App:** http://banking-platform.example/
- **Argo CD:** http://banking-platform.example/argocd/  (trailing slash required)
...
| `banking-platform.example/argocd/` hits the app, not Argo | `/argocd` IngressRoute missing or lower priority | Confirm Phase 4's IngressRoute exists; `/argocd` (more specific) should win over `/`. |
