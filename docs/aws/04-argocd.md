...
stringData:
  type: git
  url: https://github.com/Amith-Ganta/banking_application_eks.git
  username: Amith-Ganta
  password: ${GH_PAT}
EOF
```
...
`https://github.com/Amith-Ganta/banking_application_eks.git` (committed under
...
```bash
export REPO_URL="https://github.com/Amith-Ganta/banking_application_eks.git"
...
```bash
NLB=$(kubectl -n traefik get svc traefik -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')
TOKEN=$(curl -s -X POST http://$NLB/api/v1/auth/register -H "Host: banking-platform.example" \
  -d '{"email":"gitops@bank.io","password":"password123"}' | sed -n 's/.*"access_token":"\([^"]*\)".*/\1/p')
echo "token len: ${#TOKEN}"
```
(Once DNS is set in Phase 5 you'll use `https://banking-platform.example` directly.)

---

## A note on routing (path-based, single apex host)

This doc set serves **everything on the apex `banking-platform.example`** with
...
| ------- | ------------ | --- |
| App `ComparisonError: SSH agent requested but SSH_AUTH_SOCK not-specified` | `repoURL` is the `git@github.com:…` SSH form but the Secret holds an HTTPS PAT | Use `https://github.com/Amith-Ganta/banking_application_eks.git` in the Secret **and** the Application. |
